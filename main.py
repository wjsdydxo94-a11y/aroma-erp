    ...
    for file_path in excel_files:
        try:
            ...
            for sheet_name in xls.sheet_names:
                ...
                sheet_cat_override = "KT&G상품" if ("KT&G" in sheet_name or "케이티앤지" in sheet_name) else None
                for idx, row in df.iterrows():
                    vals = [str(val).strip() for val in row.values]
                    if not vals or all(v == "" or v.lower() in ["nan", "none"] for v in vals):
                        continue
                    row_str = " ".join(vals)
                    if any(kw in row_str for kw in ["회사명", "사업자", "대표", "주소", "TEL", "FAX"]):
                        continue
                    if ("코드" in row_str or "품목코드" in row_str) and ("명" in row_str or "규격" in row_str):
                        continue
                    material_code = vals[0] if len(vals) > 0 else ""
                    if not material_code or material_code.lower() in ["nan", "none", "", "품목코드", "code", "코드"]:
                        continue
                    name_kr = vals[1] if len(vals) > 1 else ""
                    name_en = vals[2] if len(vals) > 2 else ""
                    cas_no, supplier = "", ""
                    for v in vals:
                        if "-" in v and len(v) >= 7 and any(char.isdigit() for char in v):
                            parts = v.split("-")
                            if len(parts[0]) >= 2 and parts[0].isdigit():
                                cas_no = v
                        if any(kw in v for kw in ["주식회사", "코퍼레이션", "AROMA", "LLC", "주", "유한"]):
    ...
    <head>
        ...
        <style>
            * { box-sizing: border-box; margin: 0; padding: 0; }
            ...
            .aroma-sidebar { width: 260px; height: 100vh; background-color: #1E242B; color: #E2E8F0; display: flex; flex-direction: column; box-shadow: 4px 0 20px rgba(0, 0, 0, 0.15); flex-shrink: 0; }
            ...
            .aroma-sidebar summary { padding: 11px 14px; font-size: 13.5px; font-weight: 500; cursor: pointer; color: #CBD5E1; list-style: none; border-radius: 6px; display: flex; align-items: center; justify-content: space-between; }
            ...
            .aroma-sidebar li a { display: block; padding: 7px 12px; font-size: 12.5px; color: #94A3B8; text-decoration: none; border-radius: 4px; cursor: pointer; transition: all 0.2s ease; }
            .aroma-sidebar li a:hover { color: #FFFFFF; background-color: rgba(0, 168, 255, 0.12); padding-left: 15px; }
            .aroma-sidebar li a.active { color: #FFFFFF; background: linear-gradient(90deg, rgba(0, 168, 255, 0.25) 0%, rgba(0, 119, 255, 0.05) 100%); border-left: 3px solid #00A8FF; padding-left: 12px; font-weight: 600; }
    ...
    <body>
        <aside class="aroma-sidebar">
          ...
          <nav class="sidebar-nav">
            ...
            <details open>
              ...
              <ul><li><a onclick="switchTab('ktng-tab')">KT&G 상품 품목리스트</a></li></ul>
            </details>
            <details open>
        ...
        <main class="main-content">
            ...
            <div id="materials-tab" class="tab-content">
                ...
                <div class="card">
                    ...
                    <table>
                        ...
                        <tbody id="materialTableBody"><tr><td colspan="9" style="text-align: center;">데이터를 불러오는 중...</td></tr></tbody>
                    </table>
            ...
            <div id="ktng-tab" class="tab-content">
                <div class="card">
                    <h1>KT&G 상품 품목리스트</h1>
                    <p>원료 마스터 중 KT&G상품으로 분류된 품목 리스트입니다.</p>
                </div>
                <div class="card">
                    ...
                    <table>
                        ...
                        <tbody id="ktngMaterialTableBody"><tr><td colspan="8" style="text-align: center;">데이터를 불러오는 중...</td></tr></tbody>
                    </table>
            ...
            <div class="modal-content">
                ...
                <div class="modal-body">
                    ...
                    <div style="max-height: 250px; overflow-y: auto;">
                        <table>
                            <thead><tr><th>연도 선택</th><th>예계량 (kg)</th><th>관리</th></tr></thead>
        ...
        <div id="lotStockModal" class="modal-overlay">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    ...
                    <div style="max-height: 250px; overflow-y: auto;">
                        <table>
                            <thead><tr><th>LOT 번호</th><th>재고수량 (kg)</th><th>관리</th></tr></thead>
        ...
        <div id="createModal" class="modal-overlay">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <div class="form-group"><label>제품코드 *</label><input type="text" id="new_code" placeholder="예: 1-999 또는 AR-001" oninput="onMaterialCodeInput(this.value)"></div>
        ...
        <script>
            ...
            function toggleBomSelectAll(source) {
                document.querySelectorAll('.bom-row-checkbox').forEach(cb => cb.checked = source.checked);
            }
            ...
            function deleteSelectedBomMaterials() {
                const selectedIds = Array.from(document.querySelectorAll('.bom-row-checkbox:checked')).map(cb => parseInt(cb.value));
                if (selectedIds.length === 0) {
                    alert("삭제할 항목을 체크박스로 하나 이상 선택해 주세요.");
                    return;
                }
                ...
                .then(data => {
                    ...
                    data.data.forEach((row, index) => {
                        ...
                        tr.innerHTML = `
                            ...
                            <td><button class="btn-action" onclick='openEditModal(${JSON.stringify(row)})' style="padding:5px 8px;">상세</button></td>
                        `;
                        tbody.appendChild(tr);
                    });
            ...
            function saveSalesQty(id) {
                ...
                .then(res => res.json())
                .then(data => {
                    if (data.status === "SUCCESS") { m.sales_qty = sales; }
                });
            }
            ...
            function openYearlyPreModal(id) {
                ...
                if (keys.length === 0) { addYearlyRow('2026', ''); } 
                else { keys.forEach(year => addYearlyRow(year, yearlyData[year])); }
            ...
            function openLotStockModal(id) {
                ...
                if (lotData.length === 0) { addLotRow('', ''); } 
                else { lotData.forEach(item => addLotRow(item.lot, item.qty)); }
                ...
                .then(data => {
                    ...
                    data.data.forEach((row, index) => {
                        ...
                        tr.innerHTML = `
                            ...
                            <td><span class="badge" style="background:#475569;">${row.category || '원재료'}</span></td>
                            <td>${row.remark || ''}</td>
                            <td><button class="btn-action" onclick="openBomModal('${row.material_code}', '${row.material_name_kr}')" style="background:#0077FF;">BOM 등록/수정</button></td>
                        `;
                        tbody.appendChild(tr);
                    });
